...
<html>
    ...
    <body>
        ...
        <%
            List<Payment> payments = (List<Payment>) session.getAttribute("pay");
    ...
    <div class="track">
        <c:choose>
            ...
            <c:when test="<%= pay.getDelivery().getStatus() == 7%>">
                <div class="step active"> <span class="icon"> <i class="fa fa-check"></i> </span> <span class="text">Đang xử lý</span> </div>
                <div class="step active"> <span class="icon"> <i class="fa fa-user"></i> </span> <span class="text"> xác nhận </span> </div>
                <div class="step active"> <span class="icon"> <i class="fa fa-truck"></i> </span> <span class="text"> đang trên đường </span> </div>
                <div class="step active"> <span class="icon"> <i class="fa fa-box"></i> </span> <span class="text">hoàn thành</span> </div>
    ...
    <ul class="row">
        <%
            ...
            List<Product> listPro = cartDao.getProductByPaymentId(pay.getId());
            for (Product pro : listPro) {
        %>
        <li class="col-md-12">
            <figure class="itemside mb-3">
                <div class="aside"><img src="<%= pro.getImage()%>" class="img-sm border"></div>
                <figcaption class="info align-self-center">
                    <p class="title"><%= pro.getName()%>  <br> </p> <span class="text-muted"><%= pro.getPrice()%> </span>
                </figcaption>
            </figure>
        </li>
        <%
    ...
    <c:choose>   
        <c:when test="<%= pay.getDelivery().getStatus() == 4%>">
            ...
            <div class="col-md-6">
                <%
                    List<CartDetail> cartDetails = cartDao.getListCartDetail(pay.getCart().getId());
   
                %>
                <a  href="MainController?action=delidetail&cartID=<%= pay.getId()%>&payID=<%= pay.getDelivery().getId()%>" class="btn btn-light" data-abc="true">   Đánh giá  </a>
                <%
                %>
                <a  href="MainController?action=chitiet&pro=<%= pay.getId()%>" class="btn btn-light" data-abc="true">  Chi tiết   </a>
            </div>
        ...
        <c:when test="<%= pay.getDelivery().getStatus() == 1%>">
            ...
            <div class="col-md-6">
                ...
                <a  href="MainController?action=chitiet&pro=<%= pay.getId()%>" class="btn btn-light" data-abc="true">  Chi tiết   </a>
                <button type="button" class="btn btn-sm bg-danger-light mr-2" data-toggle="modal" data-target="#delete_asset_<%= pay.getCart().getId()%>">

                    <i class="fas fa-trash-alt m-r-5"></i> Hủy đơn

                </button>     </div>
            <div id="delete_asset_<%= pay.getCart().getId()%>" class="modal fade delete-modal" role="dialog">
                    ...
                    <div class="modal-content">
                        <form action="Cancelpayment" method="post">
                            <div class="modal-body text-center">
                                ...
                                <input type="hidden" name="cartID" value="<%= pay.getCart().getId()%>">
                                ...
                                <div class="m-t-20">
                                    <a> <button type="submit" class="btn btn-danger" id="confirmCancel_<%= pay.getCart().getId()%>">Hủy</button></a>
