                       ...
                       <c:set var="id1" value="${prodetail[0]}" />
                        ...
                        <td>
                           <div class="comparison-item">
                                 <a href="MainController?action=com2&proid=${id2.getId()}&proid2=${id3.getId()}">  <span class="remove-item" ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></span>
                               </a> 
                               <a class="comparison-item-thumb" href="MainControllner?action=ProDetail&proid=${id1.getId()}"><img src=" ${id1.getImage()}" alt="Apple iPhone Xs Max"></a><a class="comparison-item-title" href="MainControllner?action=ProDetail&proid=${id1.getId()}" > ${id1.getName()}</a>
                                    ...
                                    <c:when test="${sessionScope.account!= null}">  
                                        <button class="btn amado-btn" name="action" value="Add to cart" type="submit" onclick="changeSizeCart()">Thêm vào giỏ hàng </button>
                                    </c:when>
                                    <c:otherwise>
                                        <button class="btn amado-btn" type="button" onclick="getPopupLogin()">Thêm vào giỏ hàng</button>
                                    </c:otherwise>
                                </c:choose>
                                </form> </div>
                        </td>
                        <td>
                           <div class="comparison-item"> 
                                <a href="MainController?action=com2&proid=${id1.getId()}&proid2=${id3.getId()}">  <span class="remove-item" ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></span>
                               </a> 
                                <a class="comparison-item-thumb" href="MainControllner?action=ProDetail&proid=${id2.getId()}"><img src=" ${id2.getImage()}" alt="Apple iPhone Xs Max"></a><a class="comparison-item-title" href="MainControllner?action=ProDetail&proid=${id2.getId()}" > ${id2.getName()}</a>
                               <form class="cart clearfix" method="get" action="addtocart">
                                    <input type="hidden" name="productId" value="${id2.getId()}">
                                      <input type="hidden" name="accId" value="${account.getId()}">
                                    <input type="hidden" name="quantity" value="1">
                                      <c:choose>
                                    <c:when test="${id2.quantity == 0}">
                                        <p>Sản phẩm hết hàng</p>
                                    </c:when>
                                    <c:when test="${sessionScope.account!= null}">  
                                        <button class="btn amado-btn" name="action" value="Add to cart" type="submit" onclick="changeSizeCart()">Thêm vào giỏ hàng </button>
...
<td>
                           <div class="comparison-item">
                                <a href="MainController?action=com2&proid=${id1.getId()}&proid2=${id2.getId()}">  <span class="remove-item" ><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></span>
                               </a> 
                                <a class="comparison-item-thumb" href="MainControllner?action=ProDetail&proid=${id3.getId()}"><img src=" ${id3.getImage()}" alt="Apple iPhone Xs Max"></a><a class="comparison-item-title" href="MainControllner?action=ProDetail&proid=${id3.getId()}" > ${id3.getName()}</a>
                                    <form class="cart clearfix" method="get" action="addtocart">
                                    <input type="hidden" name="productId" value="${id3.getId()}">
                                      <input type="hidden" name="accId" value="${account.getId()}">
                                    <input type="hidden" name="quantity" value="1">
                                      <c:choose>
                                    <c:when test="${id3.quantity == 0}">
                                        <p>Sản phẩm hết hàng</p>
                                    </c:when>
                                    <c:when test="${sessionScope.account!= null}">  
                                        <button class="btn amado-btn" name="action" value="Add to cart" type="submit" onclick="changeSizeCart()">Thêm vào giỏ hàng </button>
...
//                });
                ...
                function changeSizeCart() {
                    var xhr = new XMLHttpRequest();
                    var servletUrl = "LoginServlet"; // Replace with the actual URL
                    xhr.open("GET", servletUrl, true);
                    xhr.onreadystatechange = function () {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            var responseText = xhr.responseText;
                            console.log("Value from the servlet: " + responseText);
                            console.log(responseText.username);
                            // Send the retrieved data back to the server to set it in the session
                            setSessionValue(responseText);
                        }
                    };
                ...
                function setSessionValue(value) {
                    var xhr = new XMLHttpRequest();
                    var setSessionUrl = "/setsession"; // Replace with the URL to a servlet that sets the session attribute
                    var params = "key=dataKey&value=" + value; // Adjust key and value as needed

                    xhr.open("POST", setSessionUrl, true);
                    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");

                    xhr.onreadystatechange = function () {
                        if (xhr.readyState === 4 && xhr.status === 200) {
                            console.log("Data set in the session: " + value);
                        }
                    };
