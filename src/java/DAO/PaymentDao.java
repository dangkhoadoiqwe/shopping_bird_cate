...
public class PaymentDao {
    ...
    public int countOrder() throws Exception {
        ...
        try {
            ...
            if (con != null) {
                stm = con.prepareStatement("SELECT COUNT(paymentID) FROM Payment");
                rs = stm.executeQuery();
                if (rs.next()) {
                    count = rs.getInt(1);
                }
    ...
    public int countcancelpayment() throws Exception {
        Connection con = null;
        PreparedStatement stm = null;
        ResultSet rs = null;
        int count = 0;
        try {
            con = DBContext.getConnection();
            if (con != null) {
                stm = con.prepareStatement("SELECT COUNT(p.paymentID) AS payment_count\n"
                        + "FROM payment p\n"
    ...
    public List<Payment> getPaymentsForLast7Months() {
        ...
        String sql = "WITH MonthRange AS (\n"
                ...
                + "LEFT JOIN\n"
                + "    (SELECT\n"
                + "        CONVERT(DATE, createTime) AS createTime,\n"
                + "        total\n"
                + "    FROM\n"
                + "        Payment\n"
                + "    ) p ON YEAR(mr.sale_month) = YEAR(p.createTime) AND MONTH(mr.sale_month) = MONTH(p.createTime)\n"
                + "GROUP BY\n"
                + "    mr.sale_month\n"
                + "ORDER BY\n"
                + "    mr.sale_month;";
    ...
    public boolean CancleProductStatus(String paymentID, String cartID, String note) throws SQLException {
        Connection con = null;
        PreparedStatement stm = null;
        boolean check = false;
        try {
            ...
            if (con != null) {
                ...
                stm.setString(2, paymentID);
                check = stm.executeUpdate() > 0;

                stm = con.prepareStatement("UPDATE Payment SET paymentStatus = 3 ");
                check = stm.executeUpdate() > 0;

                stm = con.prepareStatement("UPDATE Product\n"
                        + "SET Product.quantity = Product.quantity + COALESCE(\n"
                        + "    (SELECT SUM(cd.quantity)\n"
                        + "     FROM cartdetail cd\n"
                        + "     WHERE cd.cartID = ?\n"
                        + "     AND cd.productID = Product.productID), 0);");
        ...
        try {
            ...
            if (con != null) {
                stm = con.prepareStatement("SELECT\n"
                        ...
                        + "JOIN Delivery d ON d.deliveryID = p.deliveryID\n"
                        + "WHERE a.accountID = ?\n"
    ...
    public int insertPayment(java.util.Date dateCreatePayment, int accID, int feeShip, int voucherId, int total, int status, int deliId, int cusPhone, String address, String name) {
        Connection con = null;
        PreparedStatement stm = null;
        try {
            con = DBContext.getConnection();
            String sql = "INSERT INTO Payment(createTime,feeship,voucherId,total,paymentStatus,deliveryID,cusPhone,cusAddress,cusName, accountID) values (?,?,?,?,?,?,?,?,?,?)";
            stm = con.prepareStatement(sql);
            java.sql.Date sqlDate = new java.sql.Date(dateCreatePayment.getTime());
            stm.setDate(1, sqlDate);
            stm.setInt(2, feeShip);
            stm.setInt(3, voucherId);
            stm.setInt(4, total);
            stm.setInt(5, status);
            stm.setInt(6, deliId);
            stm.setInt(7, cusPhone);
            stm.setString(8, address);
            stm.setString(9, name);
            stm.setInt(10, accID);
            stm.execute();
            return getLastPaymentID(con, stm, accID);
        } catch (ClassNotFoundException | SQLException e) {
            e.printStackTrace();
        }
        return -1;
    ...
    private int getLastPaymentID(Connection con, PreparedStatement stm, int accID) {
        try {
            String sql = "SELECT TOP 1 paymentID FROM Payment WHERE accountID = ? ORDER BY paymentID DESC";
            stm = con.prepareStatement(sql);
            stm.setInt(1, accID);
    ...
    public boolean checkQuantityCheckOut(int proId, int quantity) {
        try {
            try (Connection con = DBContext.getConnection()) {
                String sql = "SELECT CASE\n"
                        + "           WHEN ? > Product.quantity THEN 'true'\n"
                        + "		   ELSE 'false'\n"
                        + "		   END\n"
