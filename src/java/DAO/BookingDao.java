...
"ORDER BY B.bookingID DESC; -- Sắp xếp theo giảm dần của bookingID";
                ...
                try (PreparedStatement stm = con.prepareStatement(listBookingSql)) {
                    try (ResultSet rs = stm.executeQuery()) {
                        while (rs.next()) {
                            ...
                            material.setName(rs.getString(28));
                            material.setPrice(rs.getInt(29));
                ...
                try (PreparedStatement stm = con.prepareStatement(getBookingById)) {
                    ...
                    try (ResultSet rs = stm.executeQuery()) {
                        if (rs.next()) {
                            ...
                            booking.setSize(new String(rs.getString(5).getBytes("iso-8859-1"), "utf-8"));
    ...
    String updateBooking = "UPDATE BookingRequest SET "
            + "birdType = ?,"
            + "color = ?, "
            + "size = ?, "
            + "materialID = ?, "
    ...
    public boolean updateBooking(int id, int quantity, String color, String size, String birdType, 
            int materitalId, String materialCustomName, String address, String image, String note,
            int status, String subtotal, float total) {
       try {
            try (Connection con = DBContext.getConnection()) {
                try (PreparedStatement stm = con.prepareStatement(updateBooking)) {
                    stm.setString(1, birdType);
                    stm.setString(2, color);
                    stm.setString(3, size);
                    stm.setInt(4, materitalId);
                    stm.setString(5, "images/"+ image);
                    stm.setInt(6, quantity);
                    
                    stm.setString(7, subtotal);
                    stm.setFloat(8, total);
                    stm.setString(9, note);
                    stm.setString(10, address);
                    stm.setString(11, materialCustomName);
                    stm.setInt(12, id);
