...
public void doPost(HttpServletRequest request, HttpServletResponse response)
   ...
   String voucherName = new String(request.getParameter("voucherName").getBytes(StandardCharsets.ISO_8859_1), StandardCharsets.UTF_8);
    ...
    if (success) {
        // Cập nhật thành công
        // Thêm thông báo JavaScript vào trang
        PrintWriter out = response.getWriter();
        out.println("<script type=\"text/javascript\">");
        out.println("alert('  bạn đã cập nhật thành công!!');");
        out.println("location='" + request.getContextPath() + "/MainController?action=vu';");
        out.println("</script>");
    } else {
