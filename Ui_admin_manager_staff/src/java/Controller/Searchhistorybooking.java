 ...
 * @author Admin
 */
@WebServlet(name = "Searchhistorybooking", urlPatterns = {"/Searchhistorybooking"})
public class Searchhistorybooking extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet Searchhistorybooking</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet Searchhistorybooking at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<BookingRequest> searchResults = bookingDao.getBookingRequestsByTimeRangeAndAccount(startTime, endTime, accountId);
            if (searchResults.isEmpty()) {
                String mess = "Không có đơn trong khoảng thời gian này";
                request.setAttribute("message", mess);
            }
            request.setAttribute("allBooking", searchResults);
            request.getRequestDispatcher("HistoryBooking.jsp").forward(request, response);
        } catch (Exception e) {
            e.printStackTrace();
            String mess = "không có đơn trong khoảng thời gian này";
